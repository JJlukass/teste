      ...
      if (localhostDB && nameuserDB && passwordDB && name101DB) {
        try {
          ...
          const [whitelistRows] = await con.execute(
            `SELECT whitelist FROM vrp_users WHERE id = ?`,
            [idJogo]
          );
          const hasWhitelist =
          ...
          const [idExistsRows] = await con.execute(
            `SELECT COUNT(*) AS count, discord FROM vrp_users WHERE id = ?`,
            [idJogo]
          );
          ...
          const cargoARemoverId = "1165719160190414983";
          //#####################################################################################
  ...
  if (localhostDB && nameuserDB && passwordDB && name101DB) {
    try {
      ...
      const [discordIdRows] = await con.execute(
        `SELECT discord FROM vrp_users WHERE discord = ?`,
        [userId]
      );
...
client.on("guildMemberAdd", (member) => {
  const channelId = "1165719162765709315";
  ...
  const button = new Discord.ButtonBuilder()
    ...
    .setLabel("📕 REGRAS")
    .setURL(
      "https://discord.com/channels/1165719160190414980/1165719163231285362"
    )
  ...
  var role = member.guild.roles.cache.get("1165719160190414983");
